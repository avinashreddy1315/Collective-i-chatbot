...
export default function SideChatBar({ collapsed, toggleSidebar }) {
  ...
  return (
    <SideBar
      collapsed={collapsed}
      className="bg-[#fcfcfc] border-r-2 border-[#efefef]"
    >
      <SideBarHearder>
      ...
      <div className='mt-10'>
        <button
          onClick={openDialog}
          className='border-1 border-black rounded-md p-1.5 w-[100%] text-[#3d3d3d] font-semibold text-sm text-left hover:border-gray-500'
        >
          + New Chat
        </button>
      </div>
...
const SideBar = styled.div`
  ...
  @media (max-width: 1000px) {
    width: ${({ collapsed }) => (collapsed ? '0' : '50vw')};
    padding: ${({ collapsed }) => (collapsed ? '0' : '1rem')};
    position: absolute;
    z-index: 10;
    height: 100%;
  ...
  @media (max-width: 700px) {
    width: ${({ collapsed }) => (collapsed ? '0' : '70vw')};
    padding: ${({ collapsed }) => (collapsed ? '0' : '1rem')};
    position: absolute;
    z-index: 10;
    height: 100%;
